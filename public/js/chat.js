...
function onLoad() {
  ...
  socket.on("new_users", data => {
    const userExistsInDiv = document.getElementById(`user_${data._id}`);
  ...
  socket.on("message", data => {
    if (data.message.roomId === idChatRoom) {
      ...
      const div = document.getElementById("message_user");
...
function addMessage(data) {
  ...
  divMessageUser.innerHTML += `
    <span class="user_name user_name_date">
      ...
      <span class="text-sm">${dayjs(data.message.created_at).format("DD/MM/YYYY HH:mm:ss")}</span>
    </span>
    <div class="messages">
